/*
 * fi.helsinki.cs.iot.hub.api.handlers.enablers.EnablerRequestHandler
 * v0.1
 * 2015
 *
 ...
 *
 */
public class EnablerRequestHandler extends IotHubApiRequestHandler {
	
	private Map<Method, IotHubApiRequestHandler> subHandlers;
	private List<Method> supportedMethods;

	public EnablerRequestHandler() {
		this.subHandlers = new HashMap<>();
		this.subHandlers.put(Method.GET, new EnablerGetRequestHandler());
		this.subHandlers.put(Method.POST, new EnablerPostRequestHandler());
		this.subHandlers.put(Method.PUT, new EnablerPutRequestHandler());
		this.subHandlers.put(Method.DELETE, new EnablerDeleteRequestHandler());
